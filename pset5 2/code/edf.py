...
class conv2:
    ...
    def forward(self):
        s = self.stride
        batches, h, w, cin = self.x.top.shape
        hfil, wfil, cin, cout = self.k.top.shape
        hout = int((h - hfil) / s + 1)
        wout = int((w - wfil) / s + 1)
        self.Kcol = self.k.top.transpose(3, 2, 0, 1).reshape(cout, -1)
        self.Xcol = ic.im2col_indices(self.x.top, hfil, wfil, s)
        cols = self.Kcol.dot(self.Xcol)
        self.top = cols.reshape(cout, hout, wout, batches).transpose(3, 1, 2, 0)

    def backward(self):
        s = self.stride
        hfil, wfil, cin, cout = self.k.top.shape
        cols = self.grad.transpose(3, 1, 2, 0).reshape(cout, -1)
        newk = self.k.top.transpose(3, 2, 0, 1).reshape(cout, -1)

        if self.x in ops or self.x in params:
            self.x.grad += ic.col2im_indices(cols.T.dot(newk).T, self.x.top.shape, hfil, wfil, s)

        if self.k in ops or self.k in params:
            self.k.grad += cols.dot(self.Xcol.T).reshape(cout, cin, hfil, wfil).transpose(2, 3, 1, 0)
